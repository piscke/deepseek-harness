...
归档后 UI 无查看入口、也无手动取消归档控件（本期口径，记录在 README 的 Known Limitation 中）；数据与 slot 完好，恢复面只是 UI 叠在逆向 RPC 之上。逆向的注册表写入及其第一个自动调用方见 [WhatsApp 入站消息取消归档的 Agent Note](2026-08-23-inbound-message-unarchives-conversation.md)。`workspace.list` 响应形状变化是 pre-release 直改（无兼容层）。e2e（workspace-management）钉住了「归档→行消失→reload 后仍隐藏、日志仍在」的全链路；domain 层测试钉住幂等、未知 id 拒绝、跨重启恢复与旧介质默认升级。
