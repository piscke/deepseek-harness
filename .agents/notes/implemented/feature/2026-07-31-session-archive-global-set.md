...
Archived sessions have no viewing surface and no manual unarchive control (this iteration's scope; recorded as a README Known Limitation); data and accounting slots stay intact, so a restore surface is one UI surface over the inverse RPC. The inverse registry write and its first automatic caller are [the WhatsApp unarchive-on-inbound Agent Note](2026-08-23-inbound-message-unarchives-conversation.md). The `workspace.list` response shape change is a pre-release direct edit (no compatibility layer). The workspace-management e2e pins the full chain (archive → row disappears → still hidden after reload, log still present); domain tests pin idempotence, unknown-id rejection, restart recovery, and the pre-field media default upgrade.
