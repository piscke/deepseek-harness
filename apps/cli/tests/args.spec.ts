...
describe('parseDshArgs', () => {
  it('routes profile boots and the profile aliases, handing the rest to the app', () => {
    ...
    expect(parse(['whatsapp'])).toEqual({ mode: 'profile', profile: 'whatsapp', patches: [], args: [] })
    expect(parse(['whatsapp', '--patch', 'extra.yml']))
      .toEqual({ mode: 'profile', profile: 'whatsapp', patches: ['extra.yml'], args: [] })
    // The alias boots the web app, so the same app flags reach it verbatim.
    expect(parse(['whatsapp', '--port', '8080', '--no-open']))
      .toEqual({ mode: 'profile', profile: 'whatsapp', patches: [], args: ['--port', '8080', '--no-open'] })
    expect(parse(['whatsapp', '--dump-config']))
      .toEqual({ mode: 'dump-config', profile: 'whatsapp', defaultOnly: false, patches: [] })
    expect(parse(['whatsapp', '--dump-default-config']))
      .toEqual({ mode: 'dump-config', profile: 'whatsapp', defaultOnly: true, patches: [] })
  })
  ...
  it('rejects missing profile, removed flags, and contradictory inputs', () => {
    ...
    expect(exitCode(['--profile', 'x', 'whatsapp'])).toBe(1)
    expect(exitCode(['whatsapp', '--dump-config', '--dump-default-config'])).toBe(1)
    expect(exitCode(['whatsapp', '--patch='])).toBe(1)
