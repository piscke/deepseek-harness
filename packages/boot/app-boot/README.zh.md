...
| `configModulePath(configDir, specifier)` | 从配置目录自身的安装中解析模块说明符，返回可用于 import 的 `file:` URL；在那里解析不到时返回 `undefined`。`boot()` 会把以配置目录为锚点的形式暴露给 `!!js` 表达式，使 patch 能指名 profile 自己的 `dependencies` |
| `boot(binName, absoluteConfigPath, patches?, prepare?, bareModuleBaseUrl?)` | 创建根上下文，向 Loader `!!js` 配置表达式暴露 `dshHomePath(...segments)` 与 `configModulePath(specifier)` 并安装 Loader，在配置树条目挂载前执行可选的宿主准备操作（`prepare` 可以使用 Loader，也可以提供由启动器拥有的上下文插槽），再挂载并等待 include 树结算，断言所有条目均已加载并激活，最后返回根上下文——失败时 dispose（资源释放）部分构造的上下文，并以带标签的错误 reject；可选模块基准与 `mountRootInclude` 的解析语义相同 |
