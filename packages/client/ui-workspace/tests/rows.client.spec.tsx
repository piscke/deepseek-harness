...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    const idle: SessionNode = {
      ...
      runningSubagentCount: 0, completed: false, pendingContext: false, updatedAt: 0,
    }
  ...
  it('renders a selected content-search row and opens only its session', () => {
    ...
    const result: SearchResultNode = {
      ...
      completed: false, pendingContext: false,
  ...
  it.each([
    ...
    const result: SearchResultNode = {
      ...
      pendingInteraction, running: true, runningSubagentCount: 0, completed: false, pendingContext: false,
    }
  ...
  it('shows the green done dot only on a finished, unviewed session (live activity wins the slot)', () => {
    const renderRow = (over: Partial<SessionNode>) => render(
      <SessionNodeItem
        node={{
          ...
          runningSubagentCount: 0, completed: false, pendingContext: false, updatedAt: 0, ...over,
        }}
    ...
    const running = renderRow({ completed: true, pendingContext: false, running: true })
    ...
    const delegated = renderRow({ completed: true, pendingContext: false, runningSubagentCount: 1 })
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 2, completed: false, pendingContext: false, updatedAt: 0,
      }
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 1, completed: false, pendingContext: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      const row = screen.getByRole('treeitem')
  ...
  it('keeps child activity as a secondary status while user attention is primary', () => {
    const node: SessionNode = {
      ...
      running: false, runningSubagentCount: 1, completed: false, pendingContext: false, updatedAt: 0,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
    const row = screen.getByRole('treeitem')
  ...
  it('shows the green done dot on a finished search result row', () => {
    render(<SearchResultItem
      result={{
        ...
        runningSubagentCount: 0, completed: true, pendingContext: false,
      }}
  ...
  it('shows the green new-context dot on an unwatched session and on a search row', () => {
    const view = render(
      <SessionNodeItem
        node={{
          id: sid('waiting'), title: 'Ana', blank: false, running: false,
          runningSubagentCount: 0, completed: false, pendingContext: true, updatedAt: 0,
        }}
        currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t}
      />,
    )
    expect(view.container.querySelector('[data-state="done"]')).not.toBeNull()
    expect(screen.getByText('有新内容待读')).toBeTruthy()
    view.unmount()

    // Waiting context outranks the finished-run reminder: one dot, named for
    // the material the reader has not seen.
    const both = render(
      <SessionNodeItem
        node={{
          id: sid('both'), title: 'Ana', blank: false, running: false,
          runningSubagentCount: 0, completed: true, pendingContext: true, updatedAt: 0,
        }}
        currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t}
      />,
    ...
    render(<SearchResultItem
      result={{
        id: sid('result'), title: 'Ana', workspace: 'Workspace', running: false,
        runningSubagentCount: 0, completed: false, pendingContext: true,
      }}
      currentId={undefined} onOpen={vi.fn()} t={t}
    />)
    expect(screen.getByRole('treeitem').querySelector('[data-state="done"]')).not.toBeNull()
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        ...
        pendingInteraction, running: true, runningSubagentCount: 0, completed: false, pendingContext: false, updatedAt: 0,
      }
  ...
  it('completed hover card shows the Completed status line', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 0, completed: true, pendingContext: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      fireEvent.pointerEnter(screen.getByRole('treeitem').parentElement as HTMLElement)
