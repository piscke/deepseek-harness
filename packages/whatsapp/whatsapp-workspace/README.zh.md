...
有两条过滤是部署无法关闭的策略。部署自己回传的回答永不路由——`ctx.whatsapp.claimOwnEcho` 在策略被咨询之前就识别出它——因为把它送回去会用它自己的话唤醒 agent。已经投递过的消息 id 会被丢弃，因为 provider 在重连后会重放历史。

除此之外，账号本人所写与其他消息一样被路由。`fromMe` 即操作者在已配对手机上书写，这正是一个账号与自己的 harness 对话的方式，也是无需第二个号码即可试用部署的方式；只有 harness 自己的发送才是回声，被认领的也只有它们。
...
在侧栏归档一段对话只是隐藏它那一行，并不会让对话停下：provider 继续投递，会话保留自己的日志，agent 一如既往地作答。因此一条被路由的消息会在投递之前把它的会话从注册表级全局归档集合中清除（`ctx.workspaceRegistry.unarchiveSession`），那一行随即回到它从未失去的席位上——否则就会出现一段操作者无处可寻的实时往来。未被归档的对话每条消息只多一次同步集合查找，不写入任何东西。

该写入不会被等待：那一行属于显示状态，写入失败只发出警告并让对话保持归档，而不是拖慢模型或丢弃消息。真正能让一段对话安静下来的控制项是 `denyChatIds`，它直接停止路由。
...
- **回声挺不过重启** —— 可认领的发送与 seam 同生共死，因此重启后 provider 重放的回声会被当作账号所写而路由，对话于是读回自己上一条回答。
