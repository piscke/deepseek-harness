...
Index membership is not a precondition. `resolveChat` and `fetchMessages` answer for an address this connection never observed — the kind and an empty page — and refuse only a value that is not an address at all, with `WHATSAPP_UNKNOWN_CHAT`. The account can send to such a chat and mark it read, so refusing to read it would make a warm start look like a typo, and the empty page is honest: the connection retains nothing for that conversation. Its name is absent rather than substituted, because a consumer that shows an operator where a message is going must be able to say it does not know.
