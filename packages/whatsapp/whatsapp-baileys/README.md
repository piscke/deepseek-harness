...
Baileys ships no message store, so `listChats` and `fetchMessages` answer from what **this connection observed since it loaded**: a restart discards the index, which then grows as messages arrive. It is not reliably empty at connect, because WhatsApp replays offline traffic during the handshake. `listChats` orders by newest observed message; `fetchMessages` returns newest-first and pages with `before`. Per-chat retention is capped by `historyPerChat`, evicting oldest-first.

Index membership is not a precondition. `resolveChat` and `fetchMessages` answer for an address this connection never observed — the kind and an empty page — and refuse only a value that is not an address at all, with `WHATSAPP_UNKNOWN_CHAT`. The account can send to such a chat and mark it read, so refusing to read it would make a warm start look like a typo, and the empty page is honest: the connection retains nothing for that conversation.

### Names

A conversation's name comes from three places, and none of them is the message that mentions the conversation:

- **Roster streams.** `chats`, `contacts`, and `groups` upserts and updates each carry a name field; the binding subscribes to all six with one listener and reports whichever field the entry defines.
- **`pushName` on a direct message**, which is what a contact calls themselves.
- **`groupMetadata`**, asked once when a group's first message arrives without a name and the connection knows no subject for it. Concurrent asks for the same group collapse into the one lookup in flight, and the lookup never delays the message that revealed the group.

Every resolved name is published as `whatsapp/chat-named`, and only when it changes, so a reconnection that re-syncs the same roster is silent. A group whose subject cannot be read is warned about and stays unnamed — an unnamed conversation still routes, and the next roster update names it. Names are held apart from the observed-message index, so learning what a conversation is called never invents a conversation `listChats` would then report.
...
- **The Baileys binding is outside CI** — every automated test drives a socket double, and a live account confirmed the operations named above by hand, not all of them. The library is unofficial and reverse-engineered, so WhatsApp may ban the number or break the client at any time; use a dedicated test number.
...
- **Group names are absent until observed** — a group's subject is asked for once, when its first unnamed message arrives, and otherwise reaches this connection through a roster update. A lookup that fails leaves the conversation unnamed until the next update, so a consumer must treat a missing name as a legitimate state rather than an error.
