...
Baileys 在每次凭据更新时就地重写 `creds.json`，因此进程在写入中途被杀死、或两个进程共用同一目录，都会留下被截断的文件 —— 而 Baileys 自身会把它读作"没有凭据"，并以注册新设备作答，从而悄悄丢弃原有配对，并在账号的已链接设备列表中留下一个孤立条目。因此 provider 拒绝在无法解析的凭据文件上连接，报告 `WHATSAPP_AUTH_STATE_DAMAGED` 并指明该文件，使这一损失可见，而不是伪装成一个新的二维码。状态随之变为携带该消息的 `logged-out`，因为补救方式正是 `logged-out` 本就表示的那一件事：重新配对。不会尝试重开；重开只能通过丢弃配对来"解决"这一损坏。恢复需手动进行 —— 删除该目录并重新配对。
...
索引成员资格不是前置条件。`resolveChat` 与 `fetchMessages` 会为本连接从未观察过的地址作答——给出 kind 与一个空页——只有根本不是地址的值才被拒绝，报 `WHATSAPP_UNKNOWN_CHAT`。账号可以向这样的对话发送消息并标记已读，因此拒绝读取会让热启动看起来像输错了地址；而空页是诚实的：本连接没有为该对话保留任何内容。

### 名字

对话的名字来自三处，其中没有一处是提到该对话的那条消息：

- **名册流。** `chats`、`contacts` 与 `groups` 的 upsert 与 update 各自携带一个名字字段；绑定层用一个监听器订阅全部六条流，并上报条目自己定义的那个字段。
- **私聊消息上的 `pushName`**，也就是联系人对自己的称呼。
- **`groupMetadata`**，在某个群的首条消息不带名字、且本连接不知道其主题时询问一次。对同一个群的并发询问会合并为在途的那一次查询，且查询绝不推迟那条揭示该群的消息。

每一个解析出的名字都以 `whatsapp/chat-named` 发布，且仅在名字发生变化时发布，因此重连后重新同步同一份名册是静默的。主题读取失败的群会被警告并保持无名——无名的对话照样路由，下一次名册更新会给它命名。名字与观察到的消息索引分开保存，因此得知一段对话叫什么绝不会凭空造出一个 `listChats` 随后会上报的对话。
...
- **群名在被观察到之前缺失** —— 群主题在其首条无名消息到达时被询问一次，此外只通过名册更新抵达本连接。查询失败会让该对话一直无名，直到下一次更新为止，因此消费者必须把名字缺席当作合法状态，而不是错误。
